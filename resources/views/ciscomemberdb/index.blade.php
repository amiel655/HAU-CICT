...
<section class="major container 75%">
            ...
            @if($users1->role == 'Cisco')
                <div class="card card-body bg-light">
                    ...
                    <h3><a href="/accesspointmemberdb/{{$users1->id}}">{{$users1->title}}</a></h3>
                        <small>{{$users1->body}}</small>
                        </div>
                    </div>
...
@if($cisco1->role == 'Cisco')
    <div class="card card-body bg-light">
        <div class="row">
            <div class="col-md-4 col-sm-4">
            <img class="img-responsive" style="width:100%" src="/storage/cover_images/{{$cisco1->cover_image}}" alt="">
            </div>
            <div class="col-md-8 col-sm-8">
    <h3><a href="/igoara/{{$cisco1->id}}">{{$cisco1->title}}</a></h3>
            <small>Written on {{$cisco1->created_at}}  ORGANIZATION:{{$cisco1->role}}</small>
            </div>
        </div>
