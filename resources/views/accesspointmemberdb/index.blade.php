...
<section class="major container 75%">
            ...
            @if($users1->role == 'Faculty')
                ...
                <h3><a href="/igoara/{{$users1->id}}">{{$users1->title}}</a></h3>
...
@if($igoara1->role == 'Igoara')
    <div class="card card-body bg-light">
        <div class="row">
            <div class="col-md-4 col-sm-4">
            <img class="img-responsive" style="width:100%" src="/storage/cover_images/{{$igoara1->cover_image}}" alt="">
            </div>
            <div class="col-md-8 col-sm-8">
    <h3><a href="/igoara/{{$igoara1->id}}">{{$igoara1->title}}</a></h3>
            <small>Written on {{$igoara1->created_at}} by {{$igoara1->user->name}} ORGANIZATION:{{$igoara1->role}}</small>
            </div>
        </div>
